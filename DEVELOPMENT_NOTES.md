...
## 🔥 תיקונים קריטיים - 20 דצמבר 2024

### ⚠️ Bug #11: Scheduled Function רצה בשעות לא נכונות

**הבעיה:**
הפונקציה המתוזמנת `updateNextBirthdayScheduled` הוגדרה עם `'every 24 hours'` במקום שעה קבועה, מה שגרם לרשומות להיכנס למינוס ימים בין ריצות.

**תסמינים:**
- רשומות עם יום הולדת עברי מוצגות עם מינוס ימים (-1, -2) למשך שעות רבות
- דוגמה: יום הולדת ב-19/12 00:00, אבל הפונקציה רצה ב-19/12 16:46
- בין 20/12 00:00 ל-20/12 16:46 - הרשומה בדאשבורד עם "-1 ימים"

**הסיבה השורשית:**
```typescript
// ❌ קוד בעייתי:
.schedule('every 24 hours')  // רץ כל 24 שעות מהריצה הקודמת!
```

**מה קרה:**
1. **19/12 16:46** - הפונקציה רצה, עדיין יום ההולדת (0 ימים) → לא עודכן ✅
2. **20/12 00:00** - התאריך עבר, עכשיו -1 ימים, אבל הפונקציה לא רצה ❌
3. **20/12 16:46** - הפונקציה רצה סוף סוף ועדכנה (15+ שעות באיחור!)

**הפתרון:**
```typescript
// ✅ קוד מתוקן:
.schedule('every day 00:00')  // כל יום בחצות שעון ישראל
.timeZone('Asia/Jerusalem')
```

**Flow תקין:**
1. **00:00** - `updateNextBirthdayScheduled` רץ אוטומטית
2. מחפש: `next_upcoming_hebrew_birthday < nowStr`
3. מעדכן `updated_at` → מפעיל `onBirthdayWrite` trigger
4. הטריגר בודק `shouldCalculate()` → מחשב מחדש
5. `next_upcoming_hebrew_birthday` מתעדכן לשנה הבאה
6. **אף רשומה לא נכנסת למינוס ימים!**

**קובץ:**
- `functions/src/interfaces/scheduled/update-birthdays.ts` (שורה 10)

**מגבלות Scheduled Function:**
- **Batch limit:** 500 רשומות per batch (מספיק למערכת הנוכחית)
- **Timeout:** 540 שניות (9 דקות)
- **לא Idempotent:** ריצה כפולה תגרום לחישובים מיותרים (לא קריטי)

---

## 🔥 תיקונים קריטיים - 19 דצמבר 2024

### ⚠️ Bug #10: בעיית מינוס ימים בתאריכים עבריים
