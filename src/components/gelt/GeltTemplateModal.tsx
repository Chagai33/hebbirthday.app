...
export const GeltTemplateModal: React.FC<GeltTemplateModalProps> = ({
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      setError(t('gelt.templateNameRequired'));
      return;
    }
    ...
    try {
      ...
      setError(t('gelt.templateSaveError'));
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-bold text-gray-900">{t('gelt.saveTemplate')}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={t('gelt.templateNamePlaceholder')}
              disabled={isLoading}
            />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t('gelt.templateDescription')}
            </label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder={t('gelt.templateDescriptionPlaceholder')}
              rows={3}
              disabled={isLoading}
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="isDefault" className="text-sm text-gray-700 cursor-pointer">
              {t('gelt.setAsDefaultTemplate')}
            </label>
          </div>

          <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
            <p className="font-medium mb-1">{t('gelt.templateWillSave')}:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>{t('gelt.templateWillSaveAgeGroups', { count: currentAgeGroups.length })}</li>
              <li>{t('gelt.templateWillSaveBudgetConfig')}</li>
              {currentCustomGroupSettings && (
                <li>{t('gelt.templateWillSaveCustomSettings')}</li>
              )}
            </ul>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t bg-gray-50">
          ...
          >
            {isLoading ? t('common.saving') : t('gelt.saveTemplate')}
          </Button>
        </div>
      </div>
    </div>
