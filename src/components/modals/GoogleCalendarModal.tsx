...
export const GoogleCalendarModal: React.FC<GoogleCalendarModalProps> = ({ isOpen, onClose, initialStrictMode, isBlocking = false }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={isBlocking ? undefined : onClose}>
      <div ref={modalFocusRef} className="bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[calc(100vh-2rem)] overflow-y-auto p-6 animate-slide-in" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="google-calendar-modal-title">
        ...
        <div className="flex flex-col items-center gap-4 pb-20 sm:pb-4">
          ...
          {isBlocking || (isConnected && isPrimaryCalendar) ? (
            <div className="w-full bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3 text-start mb-2">
