...
export const SyncHistoryModal: React.FC<SyncHistoryModalProps> = ({ isOpen, onClose, history }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'SUCCESS': return <CheckCircle className="w-5 h-5 text-green-500" aria-hidden="true" />;
      case 'PARTIAL': return <AlertTriangle className="w-5 h-5 text-amber-500" aria-hidden="true" />;
      case 'FAILED': return <XCircle className="w-5 h-5 text-red-500" aria-hidden="true" />;
      default: return <Clock className="w-5 h-5 text-green-700" aria-hidden="true" />;
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div
        ...
        className="bg-white rounded-xl shadow-2xl max-w-md w-full p-0 border border-gray-200 overflow-hidden max-h-[80vh] flex flex-col"
      >
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          <h3 id="sync-history-title" className="text-lg font-bold text-gray-900">{t('googleCalendar.syncHistory')}</h3>
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 p-3 rounded-full hover:bg-gray-200 transition-colors"
            ...
            <ul className="space-y-3">
              {history.map((item, index) => (
                <li key={item.id || index}>
                  <div className="border border-gray-100 rounded-lg p-3 bg-white shadow-sm hover:shadow-md transition-shadow">
